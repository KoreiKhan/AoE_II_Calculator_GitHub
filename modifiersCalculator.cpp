...
void modifiersCalculator::setAdditionalValues(int &inputPlayerNumber, int &inputPlayerAge, int *inputTechnologies, int *inputEvents){
...
void modifiersCalculator::applyEventCardEffects(){
	...
	if(playerEvents[31] == 1){
		// [31] Spirits of the Ancestors - Target unit may not attack or move this turn. For unit owner's next turn, the unit get +3 standardDamage (skip for now)
		std::cout << "Is it player " << playerNumber << "'s next turn in regards to Spirits of the Ancestors? Enter 1 if so" << "\n";
			std::cin >> isThisRelevant;
